...
metadata {
 ...
 // attribute "illuminance", "number";                         // lux
    ...
    attribute "outside", "string"  
    attribute "cloudConditions", "enum"
    attribute "currentConditions", "enum"
    attribute "moonPhase", "enum"
    attribute "currentIcon", "string"

    command "setCurrentConditions", ["enum"]
    command "setCurrentIcon", ["string"]
    command "setCloudConditions", [[name:"currentConditions",type:"ENUM", constraints:["Sunrise","Sunny","Cloudy","Mostly Sunny","Mostly Cloudy","Partly Sunny","Partly Cloudy","Sunset"]]]
    command "setPresence", ["enum"]
    command "setMoonPhase", ["enum"]
...
private Object htmlGetRepository() {
  ...
  try {
    ...
    String repositoryText = "https://cburges2.github.io/ecowitt.github.io/ecowitt.json".toURL().getText();
    if (repositoryText) {
      // text -> json
...
def setCurrentConditions(value) {
    sendEvent(name: "currentConditions", value: value)
}

def setCurrentIcon(img) {
    String iconPath = "https://cburges2.github.io/ecowitt.github.io/Dashboard%20Icons/"
    sendEvent(name: "currentIcon", value: "<img class='icon' src='${iconPath}${img}' />")
}

def setCloudConditions(value) {
    sendEvent(name: "cloudConditions", value: value)
}

def setMoonPhase(value) {
    sendEvent(name: "moonPhase", value: value)
}
