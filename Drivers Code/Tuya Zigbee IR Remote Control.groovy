 ...
 *
 * The learn and SendStoredCode commands consist of a back-and-forth sequence of command messages between
 ...
 * Modified Feb 2025 by cburgess to conform to exiting Broadlink driver command names, and to add reps to sendCode()
 */
...
/*         command "SendStoredCode", [
            [name: "Code*", type: "STRING", description: "Name of learned code or raw Base64 bytes of code to send"]
        ] */
        command "SendStoredCode", [
            [name: "Code*", type: "STRING", description: "Name of learned code or raw Base64 bytes of code to send"],[name: "Reps", type: "NUMBER", description: "Number of times to repeat the code"]
        ]        
        command "forgetCode", [
...
def SendStoredCode(final String codeNameOrBase64CodeInput) {
    info "SendStoredCode(${codeNameOrBase64CodeInput}, ${reps}"
...
def SendStoredCode(final String codeNameOrBase64CodeInput, reps) {
    info "SendStoredCode(${codeNameOrBase64CodeInput}, ${reps})"
...
def handleDoneReceiving(final Map message) {
    ...
    // We remove all whitespace in SendStoredCode to undo this
