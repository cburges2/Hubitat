...
def manageCycle(){
    ...
    //def stopHysteresis = (hysteresis - 0.15)
    def stopHysteresis = (device.currentValue("stopHysteresis").toBigDecimal())
    logDebug "stopHysteresis set to (${stopHysteresis})"
	def heatingOn = (temperature <= (heatingSetpoint - hysteresis))
	if (thermostatOperatingState == "heating") heatingOn = (temperature <= (heatingSetpoint + stopHysteresis))
